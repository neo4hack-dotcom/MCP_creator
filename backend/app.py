...
def create_app() -> FastAPI:
    app = FastAPI(
        title="MCP Creator API",
        description="Backend API for the MCP Creator guided app.",
        version="0.1.0",
    )
            ...
            if settings.model:
                try:
                    ...
                    return LLMTestResponse(
                        ok=True,
                        message=(
                            "Connexion chat reussie, mais le endpoint /models n'est pas "
                            "exploitable sur ce serveur local."
                        ),
                        models=[],
